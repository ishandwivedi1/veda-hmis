...
import { getDiagnosesMaster, getDrugs, getServices, getProcedures } from '@/app/(main)/master-data/actions';
...
export default function ConsultationForm({ queueEntryId }) {
  ...
  useEffect(() => {
    (async () => {
      const [dx, dr, sv, pr] = await Promise.all([getDiagnosesMaster(), getDrugs(), getServices(), getProcedures()]);
