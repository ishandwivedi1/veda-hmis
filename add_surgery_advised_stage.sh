#!/usr/bin/env bash
# Adds 'Surgery Advised' as the first dashboard stage, before 'Awaiting
# Biometry' -- distinguishes a freshly-created case (nobody has sent for
# biometry yet) from one where biometry has been requested and is waiting
# on the technician. Requires migration_027_send_to_queue.sql to already
# be applied (uses send_case_to_department_queue()).
set -euo pipefail
...
echo "  2. git add -A && git commit -m \"Counselling: add Surgery Advised stage before Awaiting Biometry\" && git push"
