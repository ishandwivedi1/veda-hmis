#!/usr/bin/env bash
# Adds a Surgery tab to Clinical Masters (mirrors the existing Procedure
# tab pattern exactly, using the getSurgeries/addSurgery/updateSurgery/
# deleteSurgery functions already in master-data/actions.js), and wires
# Doctor's Diagnosis & Plan's Surgery field to a dropdown sourced from it
# (was free text, e.g. 'Phacoemulsification + IOL' typed by hand).
#
# Also fixed a stale hint on the Procedure tab that still described it as
...
mkdir -p "app/(main)/master-data/clinical" "app/(main)/consultation/[id]"
echo "==> Writing updated files..."
cat > "app/(main)/master-data/clinical/page.js" << 'VEDA_EOF'
'use client';
...
VEDA_EOF
echo "  wrote app/(main)/master-data/clinical/page.js"
...
echo "  2. git add -A && git commit -m \"Add Surgery tab to Clinical Masters; wire Doctor Diagnosis Surgery"
echo "     field to it\" && git push"
echo "  3. Master Data > Clinical Masters > Surgery -- add your real surgeries"
echo "     (Phacoemulsification, SICS, etc). They will then show up in Doctor"
echo "     Diagnosis & Plan's Surgery dropdown."
