...
export async function getConsultationData(queueEntryId) {
  ...
  const [
    ...
    supabase.from('surgical_cases').select('id, procedure_name, eye, status, priority, biometry_required, fitness_required, indicative_investigations, notes, decision, decision_locked, combo_group_id').eq('visit_id', visitId).neq('status', 'Cancelled').order('created_at', { ascending: false }),
  ]);
