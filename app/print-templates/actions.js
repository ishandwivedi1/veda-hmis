...
const SAMPLE_CASE_SHEET_RAW = {
  ...
  assessment: {
    ...
    glasses_type: 'Single Vision Distance', glasses_remarks: 'Anti-reflective coating advised',
  },
...
function buildOpdCaseSheetContext(settings, { patient, encounter, visit, doctor, assessment, iopReadings, examination, diagnoses, prescriptions, followup, surgicalCase }) {
  ...
  // Type of Glass / Remarks -- captured once per assessment (not per
  // refraction type), so shown once under whichever Refraction block(s)
  // are actually printing.
  const glassesType = assessment?.glasses_type || null;
  const glassesRemarks = assessment?.glasses_remarks || null;
  const hasGlassesInfo = !!(glassesType || glassesRemarks);
  ...
  return {
    ...
    glasses_remarks: glassesRemarks || '--',
