...
function buildOpdCaseSheetContext(settings, { patient, encounter, visit, doctor, assessment, iopReadings, examination, diagnoses, prescriptions, followup, surgicalCase }) {
  ...
  return {
    ...
    examExtra,
    // Side-by-side pairing in print: only render each two-column row if
    // at least one side has data, to avoid a wasted blank half.
    hasExternalOrAnterior: externalRows.length > 0 || anteriorRows.length > 0,
    hasPosteriorOrApplanation: posteriorRows.length > 0 || hasApplanation,
