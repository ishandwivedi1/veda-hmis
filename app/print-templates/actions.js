...
export async function renderOpdCaseSheetHtml(encounterId) {
  ...
  // the patient's decision at the time. A combined surgery (e.g.
  // Cataract with Anti-VEGF Injection) advises more than one procedure
  // on the same encounter -- all of them print together as one Surgery
  // Advised line, not just the most recent.
  const { data: surgicalCases } = await supabase
    .from('surgical_cases')
...
function buildOpdCaseSheetContext(settings, { patient, encounter, visit, doctor, assessment, iopReadings, examination, diagnoses, prescriptions, followup, surgicalCases = [], investigations = [], procedures = [], referrals = [] }) {
  ...
  return {
    ...
    // Surgery advice, if this encounter marked the patient for surgery
    // -- shows what was advised and which eye. A combined surgery (e.g.
    // Cataract with Anti-VEGF Injection) advises more than one
    // procedure on the same encounter -- all of them join into one
    // line ("Cataract Surgery (OD) + Anti-VEGF Injection (OD)") rather
    // than only the most recent showing. Patient Decision is
    // intentionally not printed on the case sheet.
    hasSurgery: surgicalCases.length > 0,
    surgery_procedure_name: surgicalCases.map((sc) => sc.procedure_name).join(' + ') || null,
    surgery_eye: surgicalCases.length === 1 ? (EYE_LABEL[surgicalCases[0].eye] || surgicalCases[0].eye) : null,
    isCombinedSurgery: surgicalCases.length > 1,
    combinedSurgeryProcedures: surgicalCases.map((sc) => ({ name: sc.procedure_name, eye: EYE_LABEL[sc.eye] || sc.eye })),
