...
const SAMPLE_DISCHARGE_RAW = {
  ...
  meds: [
    { name: 'Moxifloxacin 0.5%', dosage: '1 drop', frequency: 'QID', duration: '1 week', eye: 'OD' },
    {
      name: 'Prednisolone Acetate 1%', eye: 'OD',
      taper_group_id: 'sample-taper-1',
      taper_step: 1, dosage: '1 drop', frequency: 'QID', duration: '1 week',
    },
    {
      name: 'Prednisolone Acetate 1%', eye: 'OD',
      taper_group_id: 'sample-taper-1',
      taper_step: 2, dosage: '1 drop', frequency: 'TDS', duration: '1 week',
    },
    {
      name: 'Prednisolone Acetate 1%', eye: 'OD',
      taper_group_id: 'sample-taper-1',
      taper_step: 3, dosage: '1 drop', frequency: 'BD', duration: '1 week',
    },
  ],
  followups: [
...
const SAMPLE_CASE_SHEET_RAW = {
  ...
  investigations: [{ name: 'B-Scan USG', eye: 'RE', status: 'Ordered', priority: 'Routine' }, { name: 'OCT Macula', eye: 'OU', status: 'Ordered', priority: 'Urgent' }],
...
export async function renderOpdCaseSheetHtml(encounterId) {
  ...
  const { data: investigations } = await supabase.from('investigation_orders').select('*').eq('encounter_id', encounterId).order('created_at');
  const { data: procedures } = await supabase.from('plan_procedures').select('*').eq('encounter_id', encounterId).order('created_at');
  const { data: referrals } = await supabase.from('plan_referrals').select('*').eq('encounter_id', encounterId).order('created_at');
  ...
  // the patient's decision at the time. A surgery may include more than
  // one procedure (see surgical_case_procedures) -- all print together
...
function buildOpdCaseSheetContext(settings, { patient, encounter, visit, doctor, assessment, iopReadings, examination, diagnoses, prescriptions, followup, surgicalCase, caseProcedures = [], investigations = [], procedures = [], referrals = [] }) {
  ...
  return {
    ...
    // Investigations ordered during this encounter -- Cancelled ones are
    // excluded (never actually part of the plan), but Completed ones
    // still show: the case sheet documents what was advised at the
    // visit, regardless of whether it was actioned before or after
    // printing. Priority only prints when it's not the routine default,
    // so it never adds noise to the common case.
    hasInvestigations: investigations.filter((i) => i.status !== 'Cancelled').length > 0,
    investigations: investigations
      .filter((i) => i.status !== 'Cancelled')
      .map((i) => ({ name: i.name, eye: EYE_LABEL[i.eye] || i.eye, priority: i.priority && i.priority !== 'Routine' ? i.priority : null })),

    // Surgery advice, if this encounter marked the patient for surgery
    // -- shows what was advised and which eye. A surgery may include
    // more than one procedure (see surgical_case_procedures) -- all
    // print together as one line ("Cataract Surgery (OD) + Anti-VEGF
    // Injection (OD)") rather than only the primary one showing.
    // Patient Decision is intentionally not printed on the case sheet.
    ...
    hasMultipleProcedures: caseProcedures.length > 0,
    surgeryProcedureList: surgicalCase
      ? [
          { name: surgicalCase.procedure_name, eye: EYE_LABEL[surgicalCase.eye] || surgicalCase.eye },
          ...caseProcedures.map((p) => ({ name: p.procedure_name, eye: EYE_LABEL[p.eye] || p.eye })),
        ]
      : [],

    hasProcedures: procedures.length > 0,
    procedures: procedures.map((p) => ({ name: p.name, eye: EYE_LABEL[p.eye] || p.eye, notes: p.notes || null })),
    ...
    referrals: referrals.map((r) => ({ destination: r.destination, reason: r.reason || null })),

    advice: encounter.patient_instructions || null,
    followup_text: followupParts.length > 0 ? followupParts.join(' ') : null,

    // 15-day validity / free follow-up note -- printed only on New
    // Consultation case sheets, never on Follow-up, Surgery Evaluation,
    // Post-operative Review, or Emergency visits. Date is computed off
    // this visit's own date, not today's, so a case sheet reprinted
    // later still shows the same expiry it was originally valid for.
    isNewConsultation: visit?.visit_type === 'New Consultation',
    consultation_valid_until: visit?.created_at ? fmtDate(new Date(new Date(visit.created_at).getTime() + 15 * 86400000)) : '--',
  };
}
...
export async function renderDischargeSummaryHtml(episodeId) {
  ...
  const [{ data: intraop }, { data: approval }, { data: meds }, { data: followups }, { data: caseProcedures }] = await Promise.all([
    ...
    // Injection alongside a Cataract case) -- printed alongside the
    // primary procedure in the Procedure Summary section.
    sc?.id ? supabase.from('surgical_case_procedures').select('procedure_name, eye').eq('surgical_case_id', sc.id).order('created_at') : Promise.resolve({ data: [] }),
  ]);
...
function buildDischargeSummaryContext(settings, { patient, surgeon, procedureName, eye, episode, intraop, biometry, meds, followups, caseProcedures = [] }) {
  // Same shape/grouping as every other prescription print surface --
  // recovery_medications carries the identical dosage/frequency/duration/
  // taper_group_id/taper_step columns as prescriptions, just under a
  // 'name' key instead of 'drug'. Grouping here means a tapering
  // schedule prints as one coherent row (with each step's own dosage,
  // not just the first step's) instead of one raw, un-translated row
  // per step.
  const groupedMeds = groupPrescriptionsForPrint(
    meds.map((m) => ({
      drug: m.name, eye: m.eye || 'Oral', dosage: m.dosage, frequency: m.frequency, duration: m.duration,
      taper_group_id: m.taper_group_id, taper_step: m.taper_step,
    }))
  );
  ...
  return {
    ...
    hasAdditionalProcedures: caseProcedures.length > 0,
    additionalProcedures: caseProcedures.map((p) => ({ name: p.procedure_name, eye: p.eye })),
...
// ── OPD PROCEDURE SUMMARY -- handed to the patient once an OPD
//    Procedure is completed. Doctor comes from the original consultation
//    encounter's visit (where the procedure was advised), not whichever
//    "OPD Procedure Only" visit was used for check-in on the day. ──
export async function renderOpdProcedureSummaryHtml(procedureId) {
  const supabase = await createClient();
  ...
  if (!['Completed', 'Done'].includes(proc.status)) return { error: 'This OPD Procedure has not been completed yet.' };

  const visit = proc.encounters?.visits;
  const patient = visit?.patients;
  const doctor = visit?.profiles;

  let medicines = [];
  if (proc.post_procedure_encounter_id) {
    const { data: rx } = await supabase
      .from('prescriptions')
      .select('drug_name, dosage, frequency, duration, eye')
      .eq('encounter_id', proc.post_procedure_encounter_id)
      .order('created_at');
    medicines = rx || [];
  }
  ...
  const context = {
    ...
    procedure_date: fmtDate(proc.completed_at || proc.scheduled_date || proc.created_at),
    procedure_performed: proc.procedure_performed || proc.name, eye: proc.eye || '--',

    hasFindings: !!proc.findings, findings: proc.findings || '',
    instructions: proc.post_procedure_instructions || 'As advised by the doctor.',

    hasMedicines: medicines.length > 0,
    medicines: medicines.map((m) => ({
      drug_name: m.drug_name, dosage: m.dosage || '--', frequency: m.frequency || '--', duration: m.duration || '--', eye: m.eye || '--',
    })),
  };

  const template = await getPrintTemplate('opd_procedure_summary');
  const compiled = Handlebars.compile(template.html);
  return { html: compiled(context) };
}
