mkdir -p "app/components" "app/(main)/cash-management"
...
cat > "app/(main)/cash-management/actions.js" << 'EOF'
'use server';
...
cat > "app/(main)/cash-management/page.js" << 'EOF'
'use client';
...
export default function CashManagementPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setClosedToday(await isTodayClosed());
  }, []);
  ...
  return (
    <div>
      <div style={{ borderRadius: 12, padding: '14px 18px', marginBottom: 16, color: '#fff', background: closedToday ? 'linear-gradient(135deg,#374151,#1f2937)' : 'linear-gradient(135deg,#166534,#15803d)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: closedToday ? '#9ca3af' : '#4ade80', boxShadow: closedToday ? 'none' : '0 0 8px #4ade80' }}></div>
          <div>
            <div style={{ fontWeight: 700 }}>{closedToday ? 'Today is closed' : 'Today is open'}</div>
      ...
      {activeTab === 'report' && (
        <div>
          ...
          {!report?.closing ? (
            ...
            <>
              <div style={{ background: 'linear-gradient(135deg,#1e1b4b,#1d4ed8)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 16 }}>
...
echo "Cash Management module created - Today's Collection, Reconciliation, Close Day, Daily Report, History. Placed under Finance."