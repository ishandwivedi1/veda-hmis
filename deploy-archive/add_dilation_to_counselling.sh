#!/usr/bin/env bash
# Adds 'Send for Dilation' to the Counselling (M22) workspace. Same
# underlying queue mechanism as Consultation's send-out buttons, but
# looks up TODAY's queue entry for the patient rather than assuming the
# original visit is still open -- handles both same-day counselling and
# a separate return visit. If the patient hasn't checked in today, it
# returns a clear error instead of silently failing or creating a visit
# itself (that stays Front Office's job).
set -euo pipefail
...
function CaseWorkspace({ sc, onUpdate }) {
  ...
  const [sendingDilation, setSendingDilation] = useState(false);
  ...
  async function handleSendForDilation() {
    ...
    setSendingDilation(true);
    const result = await sendForDilation(sc.id);
    setSendingDilation(false);
    if (result.error) { setAncillaryMsg({ type: 'error', text: result.error }); return; }
    setAncillaryMsg({ type: 'success', text: 'Sent -- patient will show as Awaiting Dilation in the Optometry/Doctor queue.' });
  }
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <button className="btn btn-sm" onClick={handleSendForDilation} disabled={sendingDilation}>
          <i className="ti ti-eye"></i> {sendingDilation ? 'Sending...' : 'Send for Dilation'}
        </button>
        {ancillaryMsg && (
...
echo "  2. git add -A && git commit -m \"Counselling: add Send for Dilation\" && git push"
