    ...
    <tr>
      <td style="width: 50%; vertical-align: top; padding-right: 8px;">
        ...
        <div style="border: 1px solid #999; border-top: none; border-radius: 0 0 6px 6px; padding: 8px 10px;">
          ...
          <div style="margin-bottom: 8px; padding-bottom: 8px; {{#unless @last}}border-bottom: 1px dashed #ccc;{{/unless}}">
            <div style="font-size: 10.5px; font-weight: 700; color: #1e4e8c; margin-bottom: 4px;"><i>{{device}}</i></div>
      ...
      <td style="width: 50%; vertical-align: top; padding-left: 8px;">
        ...
        <div style="border: 1px solid #999; border-top: none; border-radius: 0 0 6px 6px; padding: 8px 10px;">
          ...
          {{#each leSets}}
          <div style="margin-bottom: 8px; padding-bottom: 8px; {{#unless @last}}border-bottom: 1px dashed #ccc;{{/unless}}">
            <div style="font-size: 10.5px; font-weight: 700; color: #0d9488; margin-bottom: 4px;"><i>{{device}}</i></div>
            <table style="width: 100%; font-size: 11.5px;">
              <tr><td style="color: #555; padding: 1px 0;">Axial Length</td><td style="text-align: right; font-weight: 600;">{{axl}} mm</td></tr>
              <tr><td style="color: #555; padding: 1px 0;">K1</td><td style="text-align: right; font-weight: 600;">{{k1}} D</td></tr>
              <tr><td style="color: #555; padding: 1px 0;">K2</td><td style="text-align: right; font-weight: 600;">{{k2}} D</td></tr>
...
export async function renderBiometryReportHtml(recordId) {
  ...
  let surgeon = null;
  if (record.surgeon_id) {
    const { data: doc } = await supabase.from('profiles').select('full_name, registration_no').eq('id', record.surgeon_id).maybeSingle();
    surgeon = doc;
  }
...
function buildOpdCaseSheetContext(settings, { patient, encounter, visit, doctor, assessment, iopReadings, examination, diagnoses, prescriptions, followup }) {
  ...
  const allExamFindings = examination ? [
    ...
    ...summarizeExamRegion(examination.posterior_findings, ['Vitreous', 'Disc', 'Macula', 'Vessels', 'Peripheral Retina', 'CDR']),
  ] : [];
  ...
  // Gonioscopy -- Angle / PTM / Iris Configuration, RE and LE, each
  // dilation pass shown separately.
  const GONIO_LABELS = { angle: 'Angle Configuration', ptm: 'PTM Configuration', iris: 'Iris Configuration' };
  const gonioscopyLines = [];
  [['without', 'Without Dilation'], ['with', 'With Dilation']].forEach(([stageKey, stageLabel]) => {
    const stageData = examination?.gonioscopy_findings?.[stageKey];
    if (!stageData) return;
    Object.keys(GONIO_LABELS).forEach((key) => {
      const re = stageData[`${key}_re`];
      const le = stageData[`${key}_le`];
      if (re || le) gonioscopyLines.push({ label: `${GONIO_LABELS[key]} (RE/LE) -- ${stageLabel}`, value: `${re || '--'} / ${le || '--'}` });
    });
  });
