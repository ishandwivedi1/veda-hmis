#!/usr/bin/env bash
# Fixes the duplicate-record bug: getOrCreateBiometryRecord only
# recognized in-flight statuses as "existing", so reopening an
# already-Approved patient (e.g. from the Queue) silently created a
# second, blank biometry_records row for the same visit -- this is what
# caused Ratan Tata to show different data in different tabs/sessions.
# The duplicate row for that specific patient has already been removed
# directly in Supabase (it was empty, no attachments/versions attached).
#
# This script fixes the code so it can't happen again, and makes the
# Queue correctly show "Approved" (green badge, "View" button) instead
# of looking untouched for patients who are already done.
#
# Usage: run from the ROOT of your veda-hmis repo checkout:
#   bash deploy_biometry_duplicate_fix.sh
set -euo pipefail
...
echo "Files written:"
wc -l "$DIR/actions.js" "$DIR/page.js"
...
echo "Then commit:"
echo "  git add \"$DIR/actions.js\" \"$DIR/page.js\""
echo "  git commit -m \"Fix duplicate biometry record bug -- reuse Approved records instead of recreating\""
echo "  git push"
echo "==================================================================="
