#!/usr/bin/env bash
# CORRECTED consultation-form.js: includes BOTH the Diagnosis & Plan
# reorder (Investigations/Diagnosis/Treatment/Patient Management) AND
# the "Send for Biometry" button, which the previous reorder script
# accidentally dropped (it was built from a stale local copy). This
# file supersedes deploy_plan_tab_reorder.sh -- just run this one.
#
# Usage: run from the ROOT of your veda-hmis repo checkout:
#   bash deploy_plan_reorder_plus_biometry_fix.sh
set -euo pipefail
...
DIR="app/(main)/consultation/[id]"
mkdir -p "$DIR"

cat > "$DIR/consultation-form.js" << 'FILEEOF'
'use client';
...
echo "Files written:"
wc -l "$DIR/consultation-form.js"
echo ""
echo "Verifying both fixes landed:"
grep -c "Send for Biometry" "$DIR/consultation-form.js" | xargs echo "  Send for Biometry button occurrences (expect 1):"
grep -c "GroupHeader num={1} color=\"var(--purple)\" title=\"Investigations\"" "$DIR/consultation-form.js" | xargs echo "  Investigations as section 1 (expect 1):"
...
echo ""
echo "==================================================================="
echo "Build passed. File written but NOT committed. Review with:"
echo "  git diff \"$DIR/consultation-form.js\""
echo "Then commit:"
echo "  git add \"$DIR/consultation-form.js\""
echo "  git commit -m \"Diagnosis & Plan reorder + restore Send for Biometry button\""
echo "  git push"
echo "==================================================================="
