...
echo "== Doctor Dashboard facelift =="
echo "   1. Doctor Queue + Intermediate Queue now side by side at the top"
echo "   2. Everything else (Visits by Type, Waiting in Optometry,"
echo "      Biometry Approvals, Medical Fitness, Completed Today) laid"
echo "      out in a 2-column grid below, instead of one long stack"
echo "   3. Added Visits by Type Today -- same widget as Front Office"

echo "-- Writing app/(main)/doctor-dashboard/actions.js --"
mkdir -p "$(dirname "app/(main)/doctor-dashboard/actions.js")"
cat > "app/(main)/doctor-dashboard/actions.js" << 'JSEOF_2007364'
'use server';
...
cat > "app/(main)/doctor-dashboard/page.js" << 'JSEOF_52719755'
'use client';
