...
echo 'Applying: fix DISCHARGE_ITEMS server-action export bug (Operation Theatre + Recovery stuck loading)...'

mkdir -p 'app/(main)/ot-recovery'

cat > 'app/(main)/ot-recovery/constants.js' << 'REC_CONST_EOF'
...
echo '  git add "app/(main)/ot-recovery/constants.js" "app/(main)/ot-recovery/actions.js" "app/(main)/ot-recovery/workspace.js"'
echo '  git commit -m "Fix DISCHARGE_ITEMS server-action export bug breaking Operation Theatre and Recovery modules"'
echo '  git push'
