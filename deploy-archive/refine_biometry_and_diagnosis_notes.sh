...
echo 'Applying: biometry eye+instructions, button moved to bottom, diagnosis notes...'

mkdir -p 'app/(main)/consultation/[id]' 'app/(main)/biometry/[id]'
...
cat > 'app/(main)/biometry/[id]/workspace.js' << 'BIO_WORKSPACE_EOF'
'use client';
...
echo '  git add "app/(main)/consultation/actions.js" "app/(main)/consultation/[id]/consultation-form.js" "app/(main)/biometry/[id]/workspace.js"'
echo '  git commit -m "Biometry: eye/instructions before send, button at bottom; diagnosis notes per S.No"'
echo '  git push'
