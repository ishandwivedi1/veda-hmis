...
#
# Fixes both bugs discussed re: surgical record keeping simplification.
# DB changes ALREADY applied directly to both your production
# (flzysyzhaecaqbmdcuao) and training (ffaddzpwnbizhvhujlse) Supabase
# projects -- no SQL to run manually.
#
# DB: biometry_records now has an OPTIONAL surgical_case_id column
# (nullable FK to surgical_cases). NULL for standalone OPD-ordered
# biometry (doctor picks the eye directly -- untouched, still works
# exactly as before). Populated only when Biometry is ordered from
# Counselling against a confirmed surgical case. Existing rows were
# conservatively backfilled (only where exactly one surgical case
# unambiguously matched by encounter_id) -- Ricky Ponting's record is
# now correctly linked.
#
# BUG 1 (root cause of the Ricky Ponting issue): Counselling's "Send for
# Biometry" no longer guesses at the eye/procedure via a visit_id lookup
# that can silently miss (visit_id changes when Counselling reopens a
# case days later; encounter_id and now surgical_case_id do not). It now
# reads eye/procedure directly off the surgical case it already has in
# hand.
#
# BUG 2 (found while investigating -- not yet triggered in your data):
# a bilateral (OU) surgical case would have written surgical_eye =
# 'Both' into biometry_records, which (a) violates the DB's own CHECK
# constraint (only RE/LE/OU allowed) and (b) even 'OU' itself is a dead
# end downstream -- verifyBiometryMeasurements explicitly rejects OU
# ("not supported for a single IOL calculation"). Fixed by fanning an OU
# case out into two records (RE + LE), the same pattern the doctor's own
# OPD biometry order already uses successfully.
#
# Also included: the FULL current schema.sql, which brings your repo
# back in sync with the live database. Earlier deploy scripts (Letterhead
# settings) edited schema.sql locally on my end but never actually
# shipped it to your repo since it wasn't in those heredocs -- your
# checked-in schema.sql was missing the 3 hospital_settings letterhead
# columns until now. This script fixes that drift too.
