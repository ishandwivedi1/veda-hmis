#!/bin/bash
set -e
echo "Deploying: revert Implant Verification/Anaesthesia to Check-In, add Save Implant + Unlock-to-Edit for check-in, header now shows surgery code"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '210px 1fr 220px', gap: 14 }}>
        ...
        <div>
          ...
          <div className="card">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {intraop?.checkin_completed_at && (
                  <button
                    type="button"
                    className="btn btn-sm"
                    style={{ background: checkinUnlocked ? 'var(--amber-lt)' : 'var(--g100)', color: checkinUnlocked ? 'var(--amber)' : 'var(--g600)', border: 'none' }}
            ...
            {intraop?.checkin_completed_at && (
              <div style={{ fontSize: 11, color: checkinUnlocked ? 'var(--amber)' : 'var(--g500)', marginBottom: 8 }}>
                <i className={`ti ${checkinUnlocked ? 'ti-lock-open' : 'ti-lock'}`}></i>{' '}
                {checkinUnlocked ? 'Editing a completed check-in -- changes save immediately.' : 'This check-in is complete. Viewing read-only for reference -- use Unlock to Edit above to correct it.'}
              </div>
            )}
