#!/bin/bash
set -e
echo "Deploying: fix Check-In/Intraop module separation -- History tab, editable check-in records, Implant/Anaesthesia moved to correct module"
...
cat > "app/(main)/ot-intraop/actions.js" << 'VEDA_EOF_2'
'use server';
...
mkdir -p "app/(main)/ot-intraop"
cat > "app/(main)/ot-intraop/workspace.js" << 'VEDA_EOF_3'
'use client';
          ...
          <div className="card">
            ...
            {CHECKIN_ITEMS.map((item, i) => (
              i === CONSENT_INDEX ? (
                ...
                <div key={i} onClick={() => toggleCheckinItem(i)} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 10px', borderRadius: 8, marginBottom: 5, fontSize: 12, border: '1px solid var(--g200)', cursor: 'pointer', background: checkinChecked[i] ? 'var(--green-lt)' : '#fff' }}>
