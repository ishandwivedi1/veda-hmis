...
export default function Workspace({ otScheduleId, onBack, restrictTab }) {
  ...
  // Persists just the implant fields, callable from Check-In directly --
  // Implant Verification is captured at check-in time now, so this data
  // needs to actually reach the database before the surgeon later
  // clicks "Surgery Complete" in Intraoperative Management (VAL-OT-003
  // checks power + serial/batch there). Without a save path here, data
  // typed into these fields during check-in was silently lost the
  // moment staff navigated away.
  async function handleSaveImplant() {
    setError(''); setOk('');
    ...
    const result = await saveIntraopDraft(otScheduleId, sc.id, {
      ...
      implant_power: imPower || null, implant_category: imCategory || null, implant_serial: imSerial || null,
      implant_expiry: imExpiry || null, implant_eye: imEye,
      variance_reason: varianceReason || null,
    });
    setSavingImplant(false);
    if (result.error) { setError(result.error); return; }
    addLog('Implant details saved');
    setOk('Implant details saved.');
    refresh();
  }
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '210px 1fr 220px', gap: 14 }}>
        ...
        <div>
          ...
          {/* Implant Verification -- captured at check-in time, ahead of
              the actual surgery, so the surgeon walks in with the plan
              already confirmed rather than filling this out mid-case. */}
          <div className="card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ border: '1.5px solid', borderColor: variancePresent ? 'var(--red)' : 'var(--green)', background: variancePresent ? 'var(--red-lt)' : 'var(--green-lt)', borderRadius: 12, padding: '10px 12px' }}>
                ...
                <div style={{ marginBottom: 6 }}>
                  ...
                  <select className="fi fi-sm" value={imEye} onChange={(e) => setImEye(e.target.value)} disabled={checkinReadOnly} style={{ borderColor: eyeMismatch ? 'var(--red)' : undefined }}>
                ...
                <div style={{ marginBottom: 6 }}>
                  ...
                  <input className="fi fi-sm" value={imPower} onChange={(e) => setImPower(e.target.value)} disabled={checkinReadOnly} style={{ borderColor: powerMismatch ? 'var(--red)' : undefined }} />
                </div>
                <div style={{ marginBottom: 6 }}>
                  <label className="flbl">IOL Category</label>
                  <select className="fi fi-sm" value={imCategory} onChange={(e) => setImCategory(e.target.value)} disabled={checkinReadOnly} style={{ borderColor: categoryMismatch ? 'var(--red)' : undefined }}>
                ...
                <div>
                  ...
                  {imIolMode === 'catalog' ? (
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <input className="fi fi-sm" placeholder="Manufacturer" value={imMfr} onChange={(e) => { setImMfr(e.target.value); setImCatalogId(''); }} disabled={checkinReadOnly} style={{ borderColor: specificIolMismatch ? 'var(--red)' : undefined }} />
                      <input className="fi fi-sm" placeholder="Model" value={imModel} onChange={(e) => { setImModel(e.target.value); setImCatalogId(''); }} disabled={checkinReadOnly} style={{ borderColor: specificIolMismatch ? 'var(--red)' : undefined }} />
                    </div>
                  )}
                </div>
              </div>
            ...
            {variancePresent && (
              <div style={{ marginBottom: 10 }}>
                ...
                <input className="fi fi-sm" value={varianceReason} onChange={(e) => setVarianceReason(e.target.value)} disabled={checkinReadOnly} placeholder="Document reason for deviation from the approved plan..." />
              </div>
            )}
            ...
            <div style={{ borderTop: '1px dashed var(--g200)', paddingTop: 10, marginBottom: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <div><label className="flbl">Serial / Batch number</label><input className="fi fi-sm" value={imSerial} onChange={(e) => setImSerial(e.target.value)} disabled={checkinReadOnly} /></div>
                <div><label className="flbl">Expiry date</label><input type="date" className="fi fi-sm" value={imExpiry} onChange={(e) => setImExpiry(e.target.value)} disabled={checkinReadOnly} /></div>
              </div>
            </div>

            {!checkinReadOnly && (
              <button className="btn btn-sm btn-primary" onClick={handleSaveImplant} disabled={savingImplant}>
                <i className="ti ti-device-floppy"></i> {savingImplant ? 'Saving...' : 'Save Implant Details'}
              </button>
            )}
          </div>
          ...
          {/* Implant Status -- read-only here; the actual Implant
              Verification form now lives in Patient Check-In, since
              that's when the plan should be confirmed. This just shows
              whether it's complete, and links straight to the record
              in Check-In if power/serial are still missing (VAL-OT-003
              requires both before Surgery Complete can go through). */}
          {biometryPlans.length > 0 && (
            <div className="card">
              <div className="card-title" style={{ marginBottom: 10 }}><i className="ti ti-disc" style={{ color: 'var(--indigo)' }}></i> Implant Status</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: (!imPower || !imSerial) ? 10 : 0 }}>
                <div style={{ border: '1.5px solid var(--g200)', borderRadius: 12, padding: '10px 12px' }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--g400)', textTransform: 'uppercase', marginBottom: 8 }}>Approved IOL Plan</div>
                  {plannedPlan ? (
                    <div style={{ fontSize: 12 }}>
                ...
                <div style={{ border: '1.5px solid', borderColor: (!imPower || !imSerial) ? 'var(--amber)' : 'var(--green)', background: (!imPower || !imSerial) ? 'var(--amber-lt)' : 'var(--green-lt)', borderRadius: 12, padding: '10px 12px' }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--g500)', textTransform: 'uppercase', marginBottom: 8 }}>Actual Implanted IOL (from Check-In)</div>
                  <div style={{ fontSize: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}><span style={{ color: 'var(--g500)' }}>Power</span><strong>{imPower || 'Not recorded'}{imPower ? ' D' : ''}</strong></div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}><span style={{ color: 'var(--g500)' }}>Serial / Batch</span><strong>{imSerial || 'Not recorded'}</strong></div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}><span style={{ color: 'var(--g500)' }}>Brand / Model</span><strong style={{ textAlign: 'right' }}>{imMfr || imModel ? `${imMfr} ${imModel}`.trim() : '--'}</strong></div>
                  </div>
                </div>
              </div>
              {(!imPower || !imSerial) && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, background: 'var(--amber-lt)', color: 'var(--amber)', padding: '8px 12px', borderRadius: 8, fontSize: 11.5 }}>
                  <span><i className="ti ti-alert-triangle"></i> Power and serial/batch must both be on file before Surgery Complete will go through (VAL-OT-003).</span>
                  <a href={`/patient-checkin?otScheduleId=${otScheduleId}`} className="btn btn-sm btn-primary" style={{ textDecoration: 'none', whiteSpace: 'nowrap' }}>
                    <i className="ti ti-arrow-right"></i> Complete in Check-In
                  </a>
                </div>
              )}
            </div>
