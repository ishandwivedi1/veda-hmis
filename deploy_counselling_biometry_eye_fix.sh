...
# Code-only fix -- no schema change needed.
#
# BUG: When Biometry was ordered from Counselling (sendForBiometry),
# the biometry_records stub it creates never carried over the
# surgical eye or procedure name from the surgical case -- both
# were left blank. The Biometry module refuses to save/verify
# measurements without a surgical eye set ("Set the surgical eye
# before verifying."), so nothing could be saved.
#
# It was worse than a simple oversight: the existing fallback logic
# elsewhere (getOrCreateBiometryRecord) looks up the surgical case
# by visit_id -- but send_case_to_department_queue() issues a FRESH
# visit token when Counselling sends a case days after the original
# consultation, so visit_id no longer matches the surgical case
# that was created back on the original visit. encounter_id is the
# only thing that stays consistent across that gap.
#
# FIX: sendForBiometry now reads procedure_name + eye directly off
# the surgical case it already has in hand (fetched by caseId, no
# extra query, no visit_id ambiguity) and writes it into the
# biometry_records stub at creation time.
#
# DATA: Ricky Ponting's existing broken record (training project
# only -- he does not exist in production) has ALREADY been fixed
# directly in the database (surgical_eye set to RE, procedure_name
# set to Phaco Cataract Surgery) -- you can save his biometry right
# now without waiting on this deploy. Checked both production and
# training for any other patients stuck the same way -- none found.
