...
#
# Counselling package selection: replaced the list of separate
# clickable package rows with a dropdown. Picking a package shows
# its details (name, origin, price, includes) in a small preview
# card, then a "Select Package" button confirms it -- same as
# before, just a dropdown instead of scrolling through rows.

cd ~/veda-hmis 2>/dev/null || true

mkdir -p "app/(main)/counselling"
cat > "app/(main)/counselling/page.js" << 'FILEEOF_counselling_page_js'
'use client';
