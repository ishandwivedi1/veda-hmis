...
# UI-only changes -- no DB migration needed, no schema changes.
#
# 1) Doctor Dashboard: Completed Today card moved up alongside
#    Doctor Queue and Intermediate Queue -- all 3 now sit in one
#    row, equally distributed (3 equal columns), each heading in
#    its own color (blue / purple / green).
# 2) Consultation form: Print Case Sheet and Print Visit Summary
#    buttons moved outside the read-only <fieldset>, so a
#    completed (locked) consultation can be printed straight
#    away -- no need to "Unlock to Edit" first.
