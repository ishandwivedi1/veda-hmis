#!/bin/bash
set -e
echo "Deploying: split IOL verification -- Check-In verifies physical IOL present vs approved, Intraop independently records what was actually implanted vs approved"

mkdir -p "app/(main)/ot-intraop"
cat > "app/(main)/ot-intraop/actions.js" << 'VEDA_EOF_1'
'use server';
...
mkdir -p "app/(main)/ot-intraop"
cat > "app/(main)/ot-intraop/workspace.js" << 'VEDA_EOF_2'
'use client';
...
echo "Files written. No further DB migration needed -- verified_iol_* columns already added directly to both Supabase projects (production + training)."
