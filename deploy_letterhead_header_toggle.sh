...
#
# Adds a "Letterhead / Pre-Printed Paper" option to Hospital Settings
# (Print Templates page):
#   - Hide header on OPD Case Sheet
#   - Hide header on Glasses Prescription
#   - Blank space to leave at top (cm) -- default 5cm
#
# When a header is hidden, that much blank space is left at the top
# of the printout instead, so it aligns with the header already
# printed on your letterhead / prescription pad.
#
# DB CHANGE: 3 new columns on hospital_settings (case_sheet_hide_header,
# glasses_rx_hide_header, print_letterhead_space_cm). This script
# ALREADY applied that migration directly to both your production
# (flzysyzhaecaqbmdcuao) and training (ffaddzpwnbizhvhujlse) Supabase
# projects -- schema.sql in the repo is updated to match, but there is
# no SQL left for you to run manually.
...
mkdir -p "app/(main)/print-templates"
cat > "app/(main)/print-templates/page.js" << 'FILEEOF_print_templates_page_js'
'use client';
...
FILEEOF_print_templates_page_js
