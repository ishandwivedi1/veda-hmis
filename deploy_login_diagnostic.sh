...
function LoginForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // TEMPORARY: showing the real error message (instead of the
      // generic one) while we track down an issue -- remove this
      // once the actual cause is confirmed fixed, since a raw error
      // message isn't normally something a login screen should show.
      console.error('Login handleSubmit failed:', err);
      setError(`Something went wrong signing in: ${err?.message || String(err)}`);
    } finally {
      setLoading(false);
    }
  }
