...
# The DB changes are ALREADY LIVE on both production and training --
# applied and functionally verified directly (tested that a non-admin
# session cannot read login_history and an Administrator can). This
# just pushes the application code.

cd ~/veda-hmis 2>/dev/null || true

mkdir -p "supabase/migrations"
cat > "supabase/migrations/033_login_history.sql" << 'FILEEOF_supabase_migrations_033_login_history_sql'
-- Login History
...
FILEEOF_supabase_migrations_033_login_history_sql
...
git add -A
git commit -m "Add Login History (Administrator only): who logged in, when, approximate location via Vercel geo headers, IP, device"
git push
