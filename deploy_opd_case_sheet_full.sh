#!/bin/bash
set -e
echo "Applying: OPD Case Sheet now includes full History, Optometry, and Examination (abnormal-only)"

cat > "app/print-templates/actions.js" << 'PYEOF_6597906779599674771'
'use server';
...
const SAMPLE_CASE_SHEET_RAW = {
  ...
  examination: {
    external_findings: {}, anterior_findings: { Lens: { re: 'NS2', le: 'NS1', re_custom: '', le_custom: '' } }, posterior_findings: {},
...
function summarizeExamRegion(findingsJson, structs) {
  ...
  structs.forEach((struct) => {
    ...
    [['re', 'RE'], ['le', 'LE']].forEach(([key, label]) => {
      ...
      if (!isNormal) {
        out.push({ structure: struct, eye: label, finding: [value, custom].filter(Boolean).join(' -- ') });
      }
    });
  });
  return out;
...
function buildOpdCaseSheetContext(settings, { patient, encounter, visit, doctor, assessment, iopReadings, examination, diagnoses, prescriptions, followup }) {
  ...
  const examFindings = examination ? [
  ...
  const examExtra = [
    { label: 'CDR (RE/LE)', value: (examination?.cdr_re || examination?.cdr_le) ? `${examination?.cdr_re || '--'} / ${examination?.cdr_le || '--'}` : null },
    { label: 'Gonioscopy (RE/LE)', value: (examination?.gonio_re || examination?.gonio_le) ? `${examination?.gonio_re || '--'} / ${examination?.gonio_le || '--'}` : null },
    { label: 'Disc Appearance', value: examination?.disc_appearance },
  ...
  return {
    ...
    hasExamination: examFindings.length > 0 || examExtra.length > 0,
    examFindings,
...
PYEOF_6597906779599674771

cat > "app/(main)/print-templates/page.js" << 'PYEOF_2892581470634458629'
'use client';
