...
#
# Fixes yesterday's "patient could not be found in OT Schedule" issue,
# and adds a fast-track for the case that caused it.
#
# DB change ALREADY applied directly to both your production
# (flzysyzhaecaqbmdcuao) and training (ffaddzpwnbizhvhujlse) Supabase
# projects -- surgical_cases got a new fitness_skip_reason column
# (mirrors the existing biometry_skip_reason). No SQL to run manually.
#
# ROOT CAUSE (confirmed): when front desk creates a "Surgery" visit,
# create_walk_in_visit() only ATTACHES it to an OT case that was already
# scheduled through the normal Doctor -> Counselling pipeline -- it
# can't create one. If nothing matches (a returning patient whose
# surgery was arranged before HMIS existed, an external referral, a
# case that skipped Counselling), that attach step silently touches
# zero rows. The visit still gets created successfully, and the patient
# is invisible in OT Schedule with no indication anything went wrong.
#
# FIX 1 -- loud instead of silent: createWalkInVisit (visits/actions.js)
# now checks whether a matching OT case was actually found for a
# Surgery-type visit. If not, the front desk sees an explicit warning
# screen (visits/new/page.js) pointing them to the new fast-track below,
# instead of the gap only surfacing later in OT.
#
# FIX 2 -- the fast-track itself: OT Schedule now has a
# "Register Surgery Directly" button (open to all staff for now, no
# role restriction, per your instruction -- there is a backlog to clear
# and it can be tightened later). It creates the surgical case and
# books the OT slot in one step, reusing the exact same book_ot_slot
# RPC Counselling already uses -- nothing duplicated. Biometry and
# medical fitness are recorded as skipped WITH A MANDATORY REASON (the
# same honest pattern Biometry already uses for biometry_skip_reason,
# now extended to fitness_skip_reason), so the case has a real audit
# trail instead of the app pretending a digital workup happened.
#
# Tested end-to-end against training before writing this script: ran
# the exact insert -> book_ot_slot sequence via SQL against a real
# patient, confirmed the resulting OT Schedule row was correct, then
# cleaned up the test data. Also caught and fixed one real bug along
# the way -- the session picker was reading a field name that doesn't
# exist in the actual OT availability RPC output.
#
...
mkdir -p "app/(main)/ot-schedule"
cat > "app/(main)/ot-schedule/page.js" << 'FILEEOF_ot_schedule_page_js'
'use client';
...
FILEEOF_ot_schedule_page_js

mkdir -p "app/(main)/visits"
cat > "app/(main)/visits/actions.js" << 'FILEEOF_visits_actions_js'
'use server';
...
FILEEOF_visits_actions_js

mkdir -p "app/(main)/visits/new"
cat > "app/(main)/visits/new/page.js" << 'FILEEOF_visits_new_page_js'
'use client';
