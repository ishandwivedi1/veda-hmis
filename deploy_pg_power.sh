...
echo "=================================================="
echo "Deploying: Optometry -- Present Glasses (PG) Power section"
echo "=================================================="
echo ""
echo "NOTE: The database columns for this (ref_pg_* and section_pg_done"
echo "on optometry_assessments) have already been applied directly to"
echo "both training and production Supabase projects. This script only"
echo "deploys the app code."
echo ""
...
cat > "app/(main)/optometry/actions.js" << 'VEDA_EOF_MARKER_9f3a'
'use server';
...
git add "app/(main)/optometry/[id]/optometry-workspace.js" "app/(main)/optometry/actions.js"

git commit -m "Optometry: add Present Glasses (PG) Power section, positioned before Refraction, matching its table style/pattern exactly"
...
echo "  - New section 2, 'Present Glasses (PG) Power', sits right before"
echo "    Refraction (which is now section 3; IOP and Additional shifted"
echo "    to 4 and 5). Progress now reads out of 5 sections instead of 4."
echo "  - Same exact table layout as Refraction: VA/SPH/CYL/AXIS x"
echo "    Dist/Near x RE/LE, the same tap-to-pick SPH/CYL/AXIS fields,"
echo "    and a Copy RE to LE checkbox -- should feel identical to use."
echo "  - Tracked as its own 'Done' badge (section_pg_done), separate"
echo "    from Refraction's, so filling in PG power doesn't get"
echo "    conflated with the actual refraction being performed."
