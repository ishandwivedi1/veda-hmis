...
# UI/logic only -- no DB migration needed. The new "Medicine
# Prescription" print template is added as a code default and will
# work immediately; it's also editable later from Hospital Settings ->
# Print Templates, same as every other print type.

cd ~/veda-hmis 2>/dev/null || true

mkdir -p "app/print-templates"
cat > "app/print-templates/actions.js" << 'FILEEOF_app_print_templates_actions_js'
'use server';
...
FILEEOF_app_print_templates_actions_js

mkdir -p "app/prescription-print/[visitId]"
cat > "app/prescription-print/[visitId]/page.js" << 'FILEEOF_app_prescription_print__visitId__page_js'
...
FILEEOF_app_prescription_print__visitId__page_js
...
git add -A
git commit -m "Pharmacy Workspace: proper billing table with doctor's plain-language dosage instructions, standalone Print Prescription (Workspace + History)"
git push
