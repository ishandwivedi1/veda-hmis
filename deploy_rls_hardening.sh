...
# The DB changes themselves are ALREADY LIVE on both production and
# training (applied and verified directly). This just saves the
# migration file into your repo so it's tracked in git history like
# every other migration -- no functional change from running this.

cd ~/veda-hmis 2>/dev/null || true

mkdir -p "supabase/migrations"
cat > "supabase/migrations/031_rls_hardening_phase1.sql" << 'FILEEOF_supabase_migrations_031_rls_hardening_phase1_sql'
...
BEGIN
  ...
  END LOOP;
END $$;
FILEEOF_supabase_migrations_031_rls_hardening_phase1_sql


echo "Migration file saved."

git add -A
git commit -m "RLS hardening phase 1: close profiles privilege-escalation gap, remove unused DELETE on financial/audit tables (already applied live to both projects)"
git push
