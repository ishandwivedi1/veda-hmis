...
#
# BUG: renderInvoiceHtml() computed EVERYTHING surgery-bill-specific --
# package name/code, surgery name/eye/surgeon, AND the package breakup
# toggle -- inside a block gated on invoice.visit_id being set. But most
# of that data does not actually need a visit_id:
#   - packageName/packageCode come from the invoice's OWN Surgery line
#     item (immutable once billed, no visit dependency).
#   - surgeryName/surgeryEye/surgeon come from manual_surgery_* fields
#     on the invoice itself -- fields that exist SPECIFICALLY to cover
#     cases with no reliable surgical-case link.
#   - the breakup lookup is keyed off packageCode, not the visit.
#
# So whenever invoice.visit_id ends up null -- which happens any time
# staff bills a surgery package without separately picking a visit from
# the dropdown in New Invoice (the "Bill Now" prefill from the Surgery
# Billing widget does NOT auto-select one), or for a case registered via
# OT Schedule's "Register Surgery Directly" that was never attached to
# any visit -- the ENTIRE surgery-bill detail block silently produced
# nothing: no package name, no surgeon, and no breakup toggle at all.
#
# FIX: the gate is now just "if (isSurgery)". The surgical_cases lookup
# (an enrichment step, not a requirement) still only runs when a
# visit_id is available; everything else -- including the breakup
# toggle -- now works correctly either way.
