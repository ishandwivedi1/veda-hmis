...
#
# NEW MODULE: Surgical Journey (/surgical-journey) -- the consolidated,
# doctor-friendly workflow discussed for small setups without a
# dedicated counsellor. One page per patient, start to discharge,
# instead of switching between 5+ separate modules.
#
# DB change ALREADY applied to both production (flzysyzhaecaqbmdcuao)
# and training (ffaddzpwnbizhvhujlse) -- surgical_cases got 4 new lean
# columns (proceed_status, iol_order_notes, last_reminder_sent_at,
# reminder_count). Verified live against real training data before this
# script was written -- no SQL to run manually.
#
# WHAT IT DOES:
#   Dashboard (/surgical-journey): "Awaiting Return" list with a manual
#   call-log button (WhatsApp reminders are deferred -- see
#   conversation -- so this is a manual "log that I called them" flow
#   for now), plus every other active case.
#
#   Case page (/surgical-journey/[id]): one scrollable accordion --
#   Investigations (biometry order + pre-op panel notes) -> Proceeding?
#   -> Package & IOL Decision -> Medical Fitness -> IOL Order & Date ->
#   Advance -> Reports (drag-and-drop) -> Day of Surgery (live status +
#   deep link) -> Notes/Follow-up log.
#
# WHAT IT DELIBERATELY DOES NOT DO: rebuild OT Intraop, OT Recovery, or
# the Medical Fitness review workflow. Those stay exactly as they are
# -- this page shows live status and deep-links into them, since that
# clinical documentation is already solid and shouldn'"'"'t be duplicated.
# It also reuses selectPackage, setDecision, referForMedicalFitness,
# markReadyForScheduling, bookOTSlot, getSurgeons, and addCaseNote
# directly from Counselling'"'"'s actions.js rather than reimplementing any
# of that logic.
#
# NOT YET DONE (flagged, not started):
#   - No "Continue to Surgical Journey" link from Doctor Consultation
#     yet -- a case created via markForSurgery (advising surgery) will
#     already show up on the new dashboard automatically, but there'"'"'s
#     no direct jump-off point from the consultation screen itself yet.
#   - Not click-tested in a browser -- only syntax-validated and
#     checked against live training data via SQL.
#
...
mkdir -p "app/(main)/surgical-journey"
cat > "app/(main)/surgical-journey/page.js" << 'FILEEOF_surgical_journey_page_js'
'use client';
...
FILEEOF_surgical_journey_page_js

mkdir -p "app/(main)/surgical-journey/[id]"
cat > "app/(main)/surgical-journey/[id]/page.js" << 'FILEEOF_surgical_journey_id_page_js'
...
FILEEOF_surgical_journey_id_page_js
...
import {
  selectPackage, getPackagesForCase,
...
import { getOTAvailability } from '@/app/(main)/ot-schedule/actions';
...
function Section({ num, color, title, done, children, defaultOpen }) {
...
export default function Workspace({ caseId }) {
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      {/* 1. INVESTIGATIONS */}
      <InvestigationsSection sc={sc} biometryRecords={data.biometryRecords} onAction={flash} />

      {/* 2. PROCEED STATUS */}
      <ProceedSection sc={sc} onAction={flash} />

      {/* 3. PACKAGE & IOL DECISION */}
      <PackageDecisionSection sc={sc} onAction={flash} />

      {/* 4. MEDICAL FITNESS */}
      <FitnessSection sc={sc} fitnessReferral={data.fitnessReferral} onAction={flash} />

      {/* 5. IOL PROCUREMENT + DATE + BOOK */}
      <IolAndBookingSection sc={sc} otSchedule={data.otSchedule} onAction={flash} />

      {/* 6. ADVANCE */}
      <Section num={6} color="var(--teal)" title="Advance Payment" done={!!sc.advance_payment_id}>
      ...
      {/* 8. DAY OF SURGERY */}
      <DayOfSurgerySection sc={sc} otSchedule={data.otSchedule} recoveryEpisode={data.recoveryEpisode} router={router} />
...
function FitnessSection({ sc, fitnessReferral, onAction }) {
  const cleared = sc.fitness_cleared || sc.fitness_required === false || fitnessReferral?.status === 'Cleared';
  return (
    <Section num={4} color="var(--red)" title="Medical Fitness" done={cleared}>
...
// ── 1. INVESTIGATIONS ──────────────────────────────────────────────
function InvestigationsSection({ sc, biometryRecords, onAction }) {
  ...
  return (
    <Section num={1} color="var(--purple)" title="Investigations (Biometry + Pre-Op Panel)" done={biometryOrdered} defaultOpen={!biometryOrdered}>
...
// ── 2. PROCEED STATUS ──────────────────────────────────────────────
function ProceedSection({ sc, onAction }) {
  const OPTIONS = [
  ...
  return (
    <Section num={2} color="var(--amber)" title="Is the patient proceeding?" done={sc.proceed_status === 'Proceeding'}>
...
// ── 3. PACKAGE & IOL DECISION ──────────────────────────────────────
function PackageDecisionSection({ sc, onAction }) {
  ...
  if (!biometryReady) {
    return (
      <Section num={3} color="var(--indigo)" title="Package &amp; IOL Decision" done={false}>
  ...
  return (
    <Section num={3} color="var(--indigo)" title="Package &amp; IOL Decision" done={!!sc.package_id && sc.decision === 'Accepted'} defaultOpen={biometryReady && !sc.package_id}>
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 6 }}>Package</div>
        {sc.master_packages ? (
          <div style={{ background: 'var(--green-lt)', border: '1px solid var(--green)', borderRadius: 8, padding: 10, display: 'flex', justifyContent: 'space-between' }}>
...
function IolAndBookingSection({ sc, otSchedule, onAction }) {
  ...
  if (otSchedule) {
    return (
      <Section num={5} color="var(--teal)" title="IOL Order &amp; Surgery Date" done>
        <div style={{ fontSize: 12.5, marginBottom: 8 }}><strong>IOL:</strong> {sc.iol_order_notes || '--'}</div>
        <div style={{ background: 'var(--green-lt)', border: '1px solid var(--green)', borderRadius: 8, padding: 10, fontSize: 12.5 }}>
          <i className="ti ti-calendar-check"></i> Booked -- {new Date(otSchedule.scheduled_date).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', day: 'numeric', month: 'short', year: 'numeric' })}, {otSchedule.master_ot_sessions?.name} session
        </div>
      </Section>
    );
  }
  ...
  return (
    <Section num={5} color="var(--teal)" title="IOL Order &amp; Surgery Date" done={false} defaultOpen={readyGateMet}>
...
function DayOfSurgerySection({ sc, otSchedule, recoveryEpisode, router }) {
  ...
  return (
    <Section num={8} color={color} title="Day of Surgery" done={!!recoveryEpisode?.discharge_date} defaultOpen={!!otSchedule}>
