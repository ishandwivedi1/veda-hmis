...
echo "=================================================="
echo "Deploying: Fix Distance still missing (SPH blank, CYL/AXIS present)"
echo "=================================================="
echo ""
...
git add "app/print-templates/actions.js"

git commit -m "Fix Distance refraction still missing on printouts when SPH is blank but CYL/AXIS/VA are filled (pure astigmatism, no spherical component -- confirmed against Krishan Gopal's real record)"
...
echo ""
echo "What was actually wrong for Krishan Gopal specifically:"
echo "  His Final Rx Distance row genuinely WAS filled in -- CYL +2.0,"
echo "  AXIS 180, VA 6/9 for both eyes -- but SPH was left blank (a pure"
echo "  astigmatism correction with no spherical component, which is a"
echo "  real and valid clinical scenario). The check deciding whether to"
echo "  show the Distance section only looked at the SPH field, so it"
echo "  silently hid a row that genuinely had data in it. Confirmed"
echo "  directly against his real record before and after the fix."
echo ""
echo "What changed:"
echo "  Every place that decides 'does this row have data' -- the"
echo "  glasses prescription, the case sheet, and the Final -> Subjective"
echo "  -> Objective fallback -- now checks SPH, CYL, AXIS, and VA"
echo "  together, not SPH alone. A row only counts as empty if every"
echo "  one of those four fields is genuinely blank."
