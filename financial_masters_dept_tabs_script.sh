...
const TABS = [...DEPTS.map((d) => ({ key: d, label: d })), { key: 'packages', label: 'Packages' }];
...
export default function FinancialMastersPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setAuditLog(await getMasterAuditLog(isPackages ? 'master_packages' : 'master_services'));
  }, [isPackages]);
  ...
  async function handleAdd() {
    ...
    const result = isPackages ? await addPackage(form) : await addService({ ...form, dept: activeTab });
  ...
  async function saveEdit(record) {
    setError(''); setSuccess('');
    const result = isPackages
      ? await updatePackage(record.id, record, editForm)
      : await updateService(record.id, record, { ...editForm, dept: record.dept });
    if (result?.error) { setError(result.error); return; }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
      ...
      <div className="card">
        <div className="card-title" style={{ marginBottom: 10 }}>
          <i className="ti ti-history" style={{ color: 'var(--g400)' }}></i> Change History -- {isPackages ? 'Packages' : 'Services'}
        </div>
