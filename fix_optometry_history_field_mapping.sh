...
echo "Applying: fix Optometry History detail viewer to match current schema (dist/near, ADD, PG, per-eye Additional Measurements)"

mkdir -p "$(dirname "app/(main)/optometry-history/[assessmentId]/assessment-viewer.js")"
cat > "app/(main)/optometry-history/[assessmentId]/assessment-viewer.js" << 'VEDA_EOF_MARKER'
'use client';
