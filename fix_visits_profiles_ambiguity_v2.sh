...
echo 'Applying: fix ALL ambiguous visits->profiles embeds (Visits list, Front Office Dashboard, Patient Timeline)...'

mkdir -p 'app/(main)/visits' 'app/(main)/front-office-dashboard' 'app/(main)/patient-timeline'
...
cat > 'app/(main)/patient-timeline/actions.js' << 'TIMELINE_ACTIONS_EOF'
'use server';
...
echo '  git add "app/(main)/visits/page.js" "app/(main)/front-office-dashboard/page.js" "app/(main)/patient-timeline/actions.js"'
echo '  git commit -m "Fix all ambiguous visits->profiles embeds after adding cancelled_by foreign key"'
echo '  git push'
