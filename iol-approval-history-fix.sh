...
echo "== Veda HMIS: IOL Approval History (search + date range, all past approvals), compact OT calendar popup, fixed Medical Fitness blocking OT booking, IOL Approval live refresh + Edit, Package selection, Biometry lock mode, duplicate-order guard, referral print simplification ==" 
...
git commit -m "IOL Approval: added collapsible History section (search by patient/UHID, date range, Edit any past approval) -- previously only same-day approvals were visible anywhere; OT Calendar standalone popup route; fixed Medical Fitness still blocking OT booking server-side; IOL Approval live 15s refresh; Package selection matches Counselling; Biometry lock mode with doctor-only edit; duplicate-order guard; Biometry queue routing fix; simplified external investigation referral print"
