...
// a combined frequency string (e.g. "1 drop, QID x1 week -> 1 drop, TDS
// x1 week -> 2 drops, BD x1 week -> 2 drops, OD x1 week, then stop") so
// a tapering schedule prints as one coherent instruction instead of N
// separate-looking prescriptions. Each step carries its own dosage
// (per-step dosage is a real field on the tapering builder -- a step
// down in frequency often comes with a step down in drop count too),
// so the dosage has to travel inline with each step rather than being
// read once from the first row, or every step after the first silently
// loses its own dosage on the printout.
