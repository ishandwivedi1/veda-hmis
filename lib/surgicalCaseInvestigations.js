...
export async function addInvestigationToSurgicalCase(supabase, { encounter_id: encounterId, patient_id: patientId, visit_id: visitId }, name, eye, options = {}) {
  ...
  // behaviour. options.confirmFreshBiometry is the answer coming back
  // from that prompt -- true means always create a new record instead
  // of reusing.
  if (name.trim().toLowerCase() === 'biometry' && patientId && visitId) {
    if (options.askBeforeReuse && !options.confirmFreshBiometry) {
    ...
    const result = await adviseBiometry(patientId, visitId, encounterId, null, !!options.confirmFreshBiometry);
