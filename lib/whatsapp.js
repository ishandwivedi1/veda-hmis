...
// "feedback_veda" -- Hindi-language review request, submitted for Meta
// approval. Sending will fail with a clear error from Meta until that
// approval comes through; no special handling needed for that here,
// since sendWhatsAppTemplate already surfaces whatever error Meta
// returns, the same way every other template's failures already do.
// The template's one variable is the patient's first name (used once,
// in the Hindi greeting line). The "Review Us" button is a static URL
// baked into the approved template itself, not a dynamic one, so no
// button component is needed in this call -- if that ever changes
// (e.g. a per-patient tracking link), this will need a `components`
// array with a button parameter added.
export async function sendReviewRequestWhatsApp({ firstName, mobile, patientId, visitId = null, meta = {} }) {
  return sendWhatsAppTemplate({
    to: mobile,
    templateName: 'feedback_veda',
    languageCode: 'hi',
    bodyParams: [
      { type: 'text', text: firstName || '' },
    ],
    meta: { ...meta, patientId: patientId || null, visitId: visitId || null },
  });
}
...
/**
 * Shared upload+send mechanics for any receipt-style template that
 * attaches a PDF as a document header. Each caller builds its own
 * bodyParams list explicitly, since payment_receipt and advance_receipt
 * no longer have the same number of fields (advance_receipt dropped
 * Invoice No. -- advances have no invoice to reference).
 */
async function sendReceiptTemplate({ templateName, bodyParams, mobile, pdfBuffer, filename, patientDbId, invoiceDbId, meta = {} }) {
...
/**
 * "payment_receipt" template -- used for Collect Payment (against an
 * existing invoice). Manual-send only, never automatic.
 * Body: Name, Amount, Invoice No, Receipt No, Date (5 fields).
 */
export async function sendPaymentReceiptWhatsApp({ name, amount, invoiceNumber, receiptNumber, date, mobile, pdfBuffer, filename, patientDbId, invoiceDbId, meta = {} }) {
  return sendReceiptTemplate({
    ...
    mobile, pdfBuffer, filename, patientDbId, invoiceDbId, meta,
  });
}

/**
 * "advance_receipt" template -- used for advance payments. Sent
 * automatically, plus available via the manual button.
 * Body: Name, Amount, Receipt No, Date (4 fields -- no Invoice No.,
 * since advance payments aren't tied to an invoice).
 */
export async function sendAdvanceReceiptWhatsApp({ name, amount, receiptNumber, date, mobile, pdfBuffer, filename, patientDbId, meta = {} }) {
  return sendReceiptTemplate({
    templateName: 'advance_receipt',
    bodyParams: [
      { type: 'text', text: name || '' },
      { type: 'text', text: formatAmountPlain(amount) },
      { type: 'text', text: receiptNumber || '' },
    ...
    mobile, pdfBuffer, filename, patientDbId, meta,
  });
}
