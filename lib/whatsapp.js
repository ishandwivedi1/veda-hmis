...
/**
 * Shared by sendPaymentReceiptWhatsApp and sendAdvanceReceiptWhatsApp --
 * both templates have identical body structure (Name, Amount, Invoice No,
 * Receipt No, Date) and both attach the receipt PDF as a document header.
 */
async function sendReceiptTemplate({ templateName, name, amount, invoiceNumber, receiptNumber, date, mobile, pdfBuffer, filename, patientDbId, invoiceDbId, meta = {} }) {
...
export async function sendPaymentReceiptWhatsApp({ name, amount, invoiceNumber, receiptNumber, date, mobile, pdfBuffer, filename, patientDbId, invoiceDbId, meta = {} }) {
  return sendReceiptTemplate({
    ...
    name, amount, invoiceNumber, receiptNumber, date, mobile, pdfBuffer, filename, patientDbId, invoiceDbId, meta,
  });
}

/**
 * "advance_receipt" template -- used for advance payments, which have
 * no associated invoice (invoiceNumber is intentionally left blank).
 * Sent automatically, plus available via the manual button.
 */
export async function sendAdvanceReceiptWhatsApp({ name, amount, receiptNumber, date, mobile, pdfBuffer, filename, patientDbId, meta = {} }) {
  return sendReceiptTemplate({
    ...
    name, amount, invoiceNumber: '', receiptNumber, date, mobile, pdfBuffer, filename, patientDbId, meta,
  });
}
