...
echo 'Applying fix: lock Patient Check-In to the exact scheduled OT day, enforced server-side (not just list filtering)'
...
mkdir -p "app/(main)/ot-intraop"
cat > "app/(main)/ot-intraop/workspace.js" << 'VEDAEOF'
'use client';
...
VEDAEOF
echo "Wrote app/(main)/ot-intraop/workspace.js"

echo 'Running next build to verify...'
npm run build

echo 'Build succeeded. Staging and committing...'
git add "app/(main)/ot-intraop/actions.js" "app/(main)/ot-intraop/workspace.js"
git commit -m "Patient Check-In: lock check-in to the exact scheduled OT day (server-side, not just dashboard filtering) -- blocks early/late/missed-day check-in even via deep link"
git push
