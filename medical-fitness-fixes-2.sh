...
echo "== Veda HMIS: Medical Fitness fixes -- external investigations visible, Hindi label bug fixed, notes fields, Cleared Today section, generalized print heading ==" 
...
git commit -m "Medical Fitness: external investigations (blood work etc, ordered from Surgical Journey) now visible with report upload; fixed literal backslash-u escape bug in Systemic History label; added Notes fields to Drug Allergies and Vital Signs (form + print); added Cleared Today section separate from History; print heading generalized to Medical Fitness for Eye Surgery with Type of Surgery row"
