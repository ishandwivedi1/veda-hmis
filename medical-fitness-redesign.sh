...
echo "== Veda HMIS: Medical Fitness redesign (auto-referral on OT booking, bilingual Fitness Form + PDF, no Refer button, no Timeline tab), OT calendar popup, IOL Approval refresh/history, Package selection, Biometry lock mode ==" 

mkdir -p "app/(main)/biometry" "app/(main)/biometry/[id]" "app/(main)/consultation" "app/(main)/counselling" "app/(main)/investigation" "app/(main)/investigation/history" "app/(main)/iol-approval" "app/(main)/medical-fitness" "app/(main)/ot-schedule" "app/(main)/patient-timeline" "app/(main)/surgical-journey" "app/(main)/surgical-journey/[id]" "app/consultation/[id]" "app/medical-fitness-print/[referralId]" "app/ot-calendar-picker" "app/print-templates" "lib"
...
cat > "app/(main)/ot-schedule/actions.js" << '__VEDA_FILE_12_END__'
'use server';
...
cat > "app/(main)/ot-schedule/ot-calendar.js" << '__VEDA_FILE_13_END__'
'use client';
...
cat > "app/(main)/ot-schedule/page.js" << '__VEDA_FILE_14_END__'
'use client';
...
cat > "app/(main)/patient-timeline/page.js" << '__VEDA_FILE_15_END__'
'use client';
...
cat > "app/(main)/surgical-journey/[id]/workspace.js" << '__VEDA_FILE_16_END__'
'use client';
...
cat > "app/(main)/surgical-journey/actions.js" << '__VEDA_FILE_17_END__'
'use server';
...
cat > "app/consultation/[id]/consultation-form.js" << '__VEDA_FILE_18_END__'
'use client';
...
cat > "app/consultation/[id]/follow-up-panel.js" << '__VEDA_FILE_19_END__'
'use client';
...
cat > "app/medical-fitness-print/[referralId]/page.js" << '__VEDA_FILE_20_END__'
...
cat > "app/ot-calendar-picker/page.js" << '__VEDA_FILE_21_END__'
'use client';
...
cat > "app/print-templates/actions.js" << '__VEDA_FILE_22_END__'
'use server';
...
cat > "lib/popup.js" << '__VEDA_FILE_23_END__'
...
git commit -m "Medical Fitness redesign: auto-enters workflow when OT date is booked (removed all Refer to Doctor / Refer Again buttons); removed Visit Timeline tab; added bilingual Medical Fitness Form for Cataract Surgery tab (systemic history, medications, allergies, vitals, investigation summary, physician certification) with Save Draft / Give Clearance / Not Fit, View Investigation Reports button, and PDF certificate for patient file; OT Calendar standalone popup; IOL Approval live refresh + Edit + History section; Surgical Journey package selection matches Counselling; Biometry lock mode with doctor-only edit; duplicate-order guard; Biometry queue routing fix; simplified external investigation referral print"
git push origin main

echo "== Done. Vercel will redeploy production and training automatically. =="
echo "== NOTE: DB-side fixes (stale trigger drop, test-data cleanup, medical_fitness_referrals.form_data column) were already applied directly to both Supabase projects and need no action here. =="
