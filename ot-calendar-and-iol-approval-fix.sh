...
echo "== Veda HMIS: OT surgery date calendar view with prior bookings, IOL Approval live refresh + Edit, Fitness after IOL order/date, Package selection, Biometry lock mode, duplicate-order guard, referral print simplification ==" 
...
cat > "app/(main)/patient-timeline/page.js" << '__VEDA_FILE_10_END__'
'use client';
...
cat > "app/(main)/surgical-journey/[id]/ot-calendar-picker.js" << '__VEDA_FILE_11_END__'
'use client';
...
git commit -m "Surgical Journey: month-calendar OT date picker showing prior bookings (booking/rescheduling); IOL Approval live 15s refresh + Edit to revise same-day approvals; Medical Fitness after IOL order & surgery date; Package selection matches Counselling; Biometry lock mode with doctor-only edit; duplicate-order guard; Biometry queue routing fix; simplified external investigation referral print"
