...
echo "== Veda HMIS: removed Surgical Consumables from Patient Check-In, added Surgery ID (SUR26-00001 format) shown across Surgical Journey and OT modules ==" 
...
git commit -m "Removed Surgical Consumables section from Patient Check-In (kept in Intraoperative Management where it belongs); added surgery_code (SUR26-00001 style unique Surgery ID, DB trigger-assigned on every insert path) shown in Surgical Journey case list/header, OT Schedule, and Patient Check-In/Intraoperative Management"
git push origin main

echo "== Done. Vercel will redeploy production and training automatically. =="
echo "== NOTE: DB-side changes (surgery_code column + trigger + backfill on both projects) were already applied directly to Supabase and need no action here. =="
