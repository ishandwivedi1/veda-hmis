...
echo 'Applying: Financial Masters tabs match Billing departments exactly; remove Package Billing tab...'

mkdir -p 'app/(main)/master-data/financial' 'app/(main)/billing'

# Package Billing tab is fully retired -- handled via Front Office widget + New Invoice prefill
rm -rf 'app/(main)/billing/package'

cat > 'app/(main)/master-data/financial/page.js' << 'FM_PAGE_EOF'
'use client';
...
cat > 'app/(main)/billing/billing-tabs.js' << 'BILLING_TABS_EOF'
'use client';
...
echo '  git add -A "app/(main)/master-data/financial/page.js" "app/(main)/billing"'
echo '  git commit -m "Financial Masters tabs match Billing departments; remove Package Billing tab"'
echo '  git push'
