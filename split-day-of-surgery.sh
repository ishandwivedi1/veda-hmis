...
echo "== Veda HMIS: split Day of Surgery in Surgical Journey into Patient Check-In + Intraoperative Management sections ==" 
...
git commit -m "Surgical Journey: split Day of Surgery into two sections -- Patient Check-In (deep-links to /patient-checkin, status reflects actual check-in completion) and Intraoperative Management (deep-links to /ot-intraop, locked until check-in is done, continues through Recovery/Post-Op status); Patient Check-In: confirmation dialog before completing check-in, Pending Check-In / Checked-In Patients dashboard sections"
